...
void checkParent(treeElement* source, treeElement* sink)
{
	if (source->parent->left != nullptr && source->parent->left->number == source->number)
	{
		source->parent->left = sink;
	}
	else
	{
		source->parent->right = sink;
	}
}

treeElement* tempInitializationLeft(treeElement* source)
{
...
treeElement* tempInitializationRight(treeElement* source)
{
...
void deleteElementByNumber(treeElement* source, int num, treeHead* tree)//delete elelement by number(realy hard to write it, but i try and it works, read comment's)
{
	...
	else
	{
		...
		else//we find it! GO delete
		{
			...
			{
				checkParent(source, source->right);
			...
			{
				checkParent(source, source->left);
			...
			else//now this element have both child
			{
				...
				if (source->right->left == nullptr)//if this element don't have ->right->left tree
				{
					...
					{
						treeElement* temp = tempInitializationLeft(source);
						if (tree->head == source)
						{
							tree->head = temp;
						}
				...
				else
				{
					...
					{
						treeElement* temp = tempInitializationRight(source);
						if (tree->head == source)
						{
							tree->head = temp;
						}
